// EXERCICIO 2 - Letra a) - Lista 1 -
//
// Utilizando a equacao parametrica do circulo para gerar os vertices,
//  desenhe um octagono.
//
// Equacao parametrica do circulo:  x = cx + r*cos(theta),  y = cy + r*sin(theta)
...
int main() {
    GLFWwindow* window = glbCreateWindow("Exercicio 2a - Octagono");
    if (!window) return -1;
    ...
    // Octagono = poligono regular de 8 lados 8 segmentos no arco completo
    std::vector<float> data = generateArcFan(0.0f, 0.0f, 0.6f, 8, 0.0f, 360.0f);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        shader.use();
        shader.setVec4("uColor", 0.6f, 0.4f, 0.9f, 1.0f);
